...
function mmtjrennumadd($parent_id,$l=1)

{
    ...
    // 是否升级送矿机
   	...
   	$a111 = M('member')->where(array('id' => $parent_id))->find();
		...
		$ljmrl = M('jyzx')->where(array('mr_user'=>$a111['username'],'datatype'=>'qgcbt','zt'=>2))->sum('cbt');
	...
	$memberGroup = M('member_group')->where(array('level'=>$nowLevel))->find();
		...
		$ztj = array(
			...
			4=>array('taishu'=>2,'kj_id'=>4),

			5=>array('taishu'=>2,'kj_id'=>4),
		...
		if(isset($ztj[$memberGroup['level']]) && $l==1){

			$data = M('product')->where(array('id'=>$ztj[$memberGroup['level']]['kj_id']))->find();

			for($i=1;$i<=$ztj[$memberGroup['level']]['taishu'];$i++){
   			        ...
   			        M('product')->where(array("id"=>$ztj[$memberGroup['level']]['kj_id']))->setDec("stock");
...

   	{
   	  ...
   	  mmtjrennumadd($a111['parent_id'],$l+1);
...
// function juheSendSMS($mobile, $code,$session_id)
function juheSendSMS($mobile, $code,$session_id){
	$url = "http://v.juhe.cn/sms/send";
	// $appkey = C('CODE_APIKEY'); //请用自己的appkey代替
	$qycode=rand(111111,999999);
	$params = array(
		'key'   => 'e6dc57b046b145c722b274f625bad2de', //您申请的APPKEY
		'mobile'    => $mobile, //接受短信的用户手机号码
		'tpl_id'    => '162726', //您申请的短信模板ID，根据实际情况修改
		'tpl_value' =>'#code#='.$qycode //您设置的模板变量，根据实际情况修改
	);
...
//     */
 ...
 function sendSMS___($mobile, $code,$session_id)
...

{
	...
	$sendurl = $smsapi."sms?u=13269854510&p=".md5('woai6239243')."&m=".$mobile."&c=".urlencode($content);
